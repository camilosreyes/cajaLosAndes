...
import pageFactory.pageCajaEscolar;
import pageFactory.pageIndex;
import pageFactory.pageMenuCliente;

public class Caso_015_Ingreso_a_modulo_caja_escolar {
	tools tools;
	pageIndex objIndex;
	pageMenuCliente objMenuCliente;
	pageCajaEscolar objCajaEscolar;

	
	List<String> errores = new ArrayList<>();
	String _directory = "scripts";
	String _class = "Caso_015_Ingreso_a_modulo_caja_escolar";
	String _method = "";
	
	String Caso = "Caso_015_Ingreso_a_modulo_caja_escolar";
	...
	@Test (priority = 2)
	public void ingresoCajaEscolar(){
		if (continuar) {			
			try {	
				System.out.println("Pasamos al paso 3");
				objMenuCliente = new pageMenuCliente(tools.getDriver());
				System.out.println("ingreso al submenu caja escolar");
				objMenuCliente.linkCajaEscolar.click();
				tools.screenshot("scripts", "Caso", "ingresoCajaEscolar");
				Thread.sleep(4000);
	...
	@Test (priority = 3)
	public void cerrarCuadro(){
		if (continuar) {			
			try {	
				System.out.println("Pasamos al paso 4");
				objCajaEscolar = new pageCajaEscolar(tools.getDriver());
				System.out.println("Cerrar cuadro emergente");
				objCajaEscolar.btnCerrarMensaje.click();
				tools.screenshot("scripts", "Caso", "cerrarCuadro");
				Thread.sleep(4000);
	...
	public void volverAlHome(){
		if (continuar) {			
			try {	
				...
				objCajaEscolar.btnVolver.click();
